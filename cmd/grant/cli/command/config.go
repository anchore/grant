...
func generateConfigWithComments(fullConfig *FullConfig) (string, error) {
	// Marshal the config to YAML first
	yamlData, err := yaml.Marshal(fullConfig)
	if err != nil {
		return "", fmt.Errorf("failed to marshal config: %w", err)
	}

	// Process the YAML to add inline comments
...
`)
	...
	for _, line := range yamlLines {
		...
		} else if strings.HasPrefix(line, "quiet:") {
...
` + line + "\n")
		} else if strings.HasPrefix(line, "ignore-packages:") {
...
` + line + " # Disable filesystem license file search\n")
		...
		} else if strings.HasPrefix(line, "only-unlicensed:") {
